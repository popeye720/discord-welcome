...
class DynamicRegisterModal(discord.ui.Modal):
    def __init__(self, guild_id, user, questions, auto_role_id=None, old_answers=None):
        ...
        for q in questions:
            # Paragraph style input for multi-line answers
...
class Forms(commands.Cog):
    ...
    @app_commands.describe(
        ...
        questions="Format: --q Question (multiline allowed) --ans",
    ...
    async def setform(
        ...
        # ---------------- PARSE MULTILINE QUESTIONS ----------------
        lines = [l.rstrip() for l in questions.splitlines() if l.strip()]
        parsed = []
        i = 0
        while i < len(lines):
            if not lines[i].startswith("--q"):
                return await interaction.response.send_message(
                    "❌ Invalid format. Each question must start with `--q`",
                    ephemeral=True
                )

            q_lines = []
            i += 1
            while i < len(lines) and lines[i] != "--ans":
                q_lines.append(lines[i])
                i += 1

            if i >= len(lines) or lines[i] != "--ans":
                return await interaction.response.send_message(
